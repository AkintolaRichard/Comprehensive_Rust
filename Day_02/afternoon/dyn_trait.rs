...
* Generics, including impl Trait, use monomorphization to create a
 ...
 full type information and can resolve which type's trait implementation
 to use.
* When using dyn Trait, it instead uses dynamic dispatch through a virtual
 method table(vtable). This means that there's a single version of
 fn dynamic that is used regardless of what type of Pet is passed in.
* When using dyn Trait, the trait object needs to be behind some kind of
 indirection. In this case it's reference, though smart pointer types like
 Box can also be used.
* At runtime, a &dyn Pet is represented as a "fat pointer", i.e. a pair of
 ...
 that type. When calling the talk method on &dyn Pet the compiler looks up
 the function pointer for talk in vtable and then invokes the function,
 pasing the pointer to the Dog or Cat into that function. The compiler
