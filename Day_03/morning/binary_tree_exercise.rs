...
children (left and right). We will create a tree where each node stores a value. For a given node N, all nodes in a N's left subtree contain smaller
...
/// A container storing a set of values, using a binary
///
