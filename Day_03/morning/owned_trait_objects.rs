...
&dyn Pet. However, we can also use trait objects with smart pointers like
Box to create an owned trait object: Box<dyn Pet>.
*/
 ...
 * Types that implement a given trait may be of different sizes. This makes
  it impossible to have things like Vec<dyn Pet> in the example above.
 ...
 * In the example, pets is allocated on the stack and the vector data is on
  the heap. The two vector elements are fat pointers:
    * A fat pointer is a double-width pointer. It has two components: a
     pointer to the actual object and a pointer to the virtual method table
